...
export default function NewPatientRegistration() {
  ...
  const fetchTimeSlots = async () => {
    try {
      const response = await fetch('https://api.varahasdc.co.in/reception/time-slots');
      if (response.ok) {
        const data = await response.json();
  ...
  const fetchPatientData = async (patientId: string) => {
    try {
      const response = await fetch(`https://api.varahasdc.co.in/reception/patients/${patientId}`);
      if (response.ok) {
        const data = await response.json();
  ...
  const handleSubmit = async (action: string) => {
    try {
      ...
      if (isEditMode && editPatientId) {
        response = await fetch(`https://api.varahasdc.co.in/reception/patients/${editPatientId}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(submitData)
        });
      } else {
        response = await fetch('https://api.varahasdc.co.in/reception/patients', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(submitData)
        });
