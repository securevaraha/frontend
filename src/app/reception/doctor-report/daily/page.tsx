...
export default function DoctorDailyReport() {
  ...
  const generateReport = async (start = startDate, end = endDate) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`https://api.varahasdc.co.in/reception/reports/doctor?startDate=${start}&endDate=${end}`);
  ...
  const exportToExcel = () => {
    ...
    const url = `https://api.varahasdc.co.in/reception/reports/doctor/excel?startDate=${startDate}&endDate=${endDate}`;
