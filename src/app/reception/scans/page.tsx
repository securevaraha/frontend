...
export default function ReceptionScans() {
  ...
  const fetchScans = async () => {
    try {
      const response = await fetch('https://api.varahasdc.co.in/admin/scans');
      if (response.ok) {
  ...
  const handleDelete = async (scan: ScanData) => {
    if (confirm(`Are you sure you want to delete ${scan.s_name}?`)) {
      try {
        const response = await fetch(`https://api.varahasdc.co.in/admin/scans/${scan.s_id}`, { method: 'DELETE' });
        if (response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const url = editingScan ? `https://api.varahasdc.co.in/admin/scans/${editingScan.s_id}` : 'https://api.varahasdc.co.in/admin/scans';
