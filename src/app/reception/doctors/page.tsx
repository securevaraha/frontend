...
export default function ReceptionDoctors() {
  ...
  const fetchDoctors = async () => {
    try {
      const response = await fetch('https://api.varahasdc.co.in/admin/doctors');
      if (response.ok) {
  ...
  const handleDelete = async (doctor: DoctorData) => {
    if (confirm(`Are you sure you want to delete Dr. ${doctor.dname}?`)) {
      try {
        const response = await fetch(`https://api.varahasdc.co.in/admin/doctors/${doctor.d_id}`, { method: 'DELETE' });
        if (response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const url = editingDoctor ? `https://api.varahasdc.co.in/admin/doctors/${editingDoctor.d_id}` : 'https://api.varahasdc.co.in/admin/doctors';
