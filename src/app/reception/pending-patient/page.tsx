...
export default function PendingPatient() {
  ...
  const fetchPendingPatients = async () => {
    try {
      const response = await fetch('https://api.varahasdc.co.in/reception/patients/list');
      if (response.ok) {
  ...
  const updatePatientStatus = async (patientId: number, newStatus: string) => {
    ...
    try {
      const response = await fetch(`https://api.varahasdc.co.in/admin/patients/${patientId}`, {
        method: 'PUT',
        headers: {
