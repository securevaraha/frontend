...
export default function DoctorScanReport() {
  ...
  const fetchDoctors = async () => {
    try {
      const response = await fetch('https://api.varahasdc.co.in/doctor/ct-scan-doctors');
      if (response.ok) {
  ...
  const fetchReports = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://api.varahasdc.co.in/superadmin/doctor-scan-report?${params}`);
      if (response.ok) {
        const data = await response.json();
