...
// Create weblogin user endpoint
router.post('/create-weblogin', async (req, res) => {
  let connection;
  try {
    connection = await mysql.createConnection(dbConfig);
    ...
    const [existingUsers] = await connection.execute(
      'SELECT admin_id FROM admin WHERE LOWER(username) = ?',
      ['weblogin']
    );
    ...
    if (existingUsers.length > 0) {
      return res.json({
        success: true,
        message: 'WebLogin user already exists',
    ...
    res.json({
      success: true,
      message: 'WebLogin user created successfully',
      user: {
    ...
    console.error('Create weblogin error:', error);
    res.status(500).json({
      success: false,
      error: 'Failed to create weblogin user',
