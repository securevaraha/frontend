...
async function sendWhatsAppReport(buffer, mimeType, fileName, patientName, contactNumber) {
  ...
  const mediaRes  = await fetch(`https://graph.facebook.com/${graphVer}/${phoneId}/media`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${token}` },
    body: form,
  });
  const mediaData = await mediaRes.json().catch(() => ({}));
  if (!mediaRes.ok) throw new Error(`WhatsApp media upload failed: ${JSON.stringify(mediaData)}`);
...
router.post('/save-console', async (req, res) => {
  ...
  try {
    ...
    // Use date_of_examination if provided, otherwise fall back to currentDate
    const scanDate = date_of_examination || currentDate;
    ...
    await connection.execute(`
      ...
      WHERE cro = ?
    `, [examination_id || null, scanDate, scanStatusValue, cro]);
  ...
  try {
    ...
    const query = `
      SELECT 
        ...
        patient_new.*,
        doctor.dname as doctor_name,
        console.report_filename,
        console.blob_url,
        console.whatsapp_sent
      FROM patient_new 
      JOIN console ON console.c_p_cro = patient_new.cro 
      JOIN doctor ON doctor.d_id = patient_new.doctor_name
...
router.post('/update-console', async (req, res) => {
  ...
  try {
    ...
    if (onWhatsappSent !== undefined) {
      consoleUpdateFields.push('whatsapp_sent = ?');
      consoleUpdateValues.push(onWhatsappSent ? 1 : 0);
    }
    if (reportFileName !== undefined) {
      consoleUpdateFields.push('report_filename = ?');
      consoleUpdateValues.push(reportFileName);
    }
...
router.post('/upload-report', upload.single('report'), async (req, res) => {
  ...
  try {
    ...
    if (!reportName || !cro || !conId) {
      return res.status(400).json({ error: 'Missing required fields: reportName, cro, conId' });
    }
    if (!file) {
    ...
    // 1. Save file to disk -> public/uploads/reports/
    const uploadDir = path.join(__dirname, '..', 'public', 'uploads', 'reports');
    if (!fs.existsSync(uploadDir)) fs.mkdirSync(uploadDir, { recursive: true });
    fs.writeFileSync(path.join(uploadDir, fileName), file.buffer);

    // Real public URL served by Express static
    ...
    connection = await mysql.createConnection(dbConfig);
    await connection.execute(
      'UPDATE console SET report_filename = ?, blob_url = ?, whatsapp_sent = ?, report_uploaded = 1 WHERE con_id = ?',
      [fileName, blobUrl, whatsappSent ? 1 : 0, conId]
    );
